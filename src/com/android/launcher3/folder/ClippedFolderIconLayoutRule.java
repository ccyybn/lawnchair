...
public class ClippedFolderIconLayoutRule {
    ...
    private void getPosition(int index, int curNumItems, float[] result) {
        ...
        float gap = (mAvailableSpace - padding * 2 - iconSize * DeviceProfile.FOLDER_PREVIEW_ROW_COLUMN) / (DeviceProfile.FOLDER_PREVIEW_ROW_COLUMN - 1) * 0.9f;
        result[0] = padding + column * (gap + iconSize);
        result[1] = padding + row * (gap + iconSize);
    }

    public float scaleForItem(int numItems) {
        return 0.356f * mBaselineIconScale;
    }
